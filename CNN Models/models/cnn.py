...
class CNN(object):
    ...
    def __init__(self, input_width, num_input_channels, num_channels, kernel_sizes, strides,
        ...
        for idx in range(len(num_channels)):
            self.convolutional_layers.append(Conv1d(in_channels, num_channels[idx], kernel_sizes[idx], strides[idx], conv_weight_init_fn, bias_init_fn))
            in_channels = num_channels[idx]
            input_width = (input_width - kernel_sizes[idx]) // strides[idx] + 1
    ...
    def forward(self, A):
        ...
        # <---------------------
        A_conv = A
        for idx in range(len(self.convolutional_layers)):
            Z_conv = self.convolutional_layers[idx].forward(A_conv)
            A_conv = self.activations[idx].forward(Z_conv)

            
        A_flatten = self.flatten.forward(A_conv)
        A_linear = self.linear_layer.forward(A_flatten)
    ...
    def backward(self, labels):
        ...
        # <---------------------
        dLdA_linear = self.linear_layer.backward(grad)
        dLdA_flatten = self.flatten.backward(dLdA_linear)

        dLdA = dLdA_flatten     # Initial derivative for the convolution

        for idx in range(len(self.convolutional_layers)-1, -1, -1):
            dLdZ = dLdA * self.activations[idx].backward()  # Using the dLdA to compute dLdZ
            dLdA = self.convolutional_layers[idx].backward(dLdZ)    # This is the dLdA for the previous layer now
