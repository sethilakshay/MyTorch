...
class CharacterPredictor(object):
    ...
    def __init__(self, input_dim, hidden_dim, num_classes):
        ...
        self.rnn = GRUCell(input_dim, self.hidden_dim)
        self.projection = Linear(self.hidden_dim, num_classes)
        self.projection.W = np.random.rand(num_classes, self.hidden_dim)
...
def inference(net, inputs):
    ...
    h_next = np.zeros(shape = net.hidden_dim)

    for idx in range(seq_len):
        logits_temp, h_next = net.forward(inputs[idx], h_next)
