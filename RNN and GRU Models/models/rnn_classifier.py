...
class RNNPhonemeClassifier(object):
    ...
    def __init__(self, input_size, hidden_size, output_size, num_layers=2):
        ...
        # The Below code is implementing multiple hidden layers for an RNN
        # At initial input layer (i=0) dimensions are passed as (input_size, hidden_size)
        # After that hidden layers start repeating itself
        # Note: Each hidden layer box means a "h" number of neurons going inside the page
        self.rnn = [
    ...
    def forward(self, x, h_0=None):
        ...
        for input_idx in range(seq_len):
            ...
            for layer, rnn_cell in enumerate(self.rnn):

                if layer == 0:
                    logits = rnn_cell.forward(x[:,input_idx,:], self.hiddens[input_idx][layer,:,:])
                    hidden_layer.append(logits)

                else:
                    logits = rnn_cell.forward(logits, self.hiddens[input_idx][layer,:,:])
                    hidden_layer.append(logits)

            hidden_layer = np.array(hidden_layer)
            self.hiddens.append(hidden_layer)

        Z_out = self.output_layer.forward(logits)
    ...
    def backward(self, delta):
        ...
        * Normalize dh by batch_size since initial hidden states are also treated
        ...
        """
        # self.hidden Dimensions = [(seq_len+1) * (num_layers, batch_size, hidden_size)]

        for input_idx in range((seq_len+1)-1, 0, -1):

            for layer in range(len(self.rnn)-1, -1, -1):

                h_t         = self.hiddens[input_idx][layer,:,:]     # Dimensions = (batch_size, hidden_size)
                h_prev_t    = self.hiddens[input_idx-1][layer,:,:]

                if layer > 0:
                    h_prev_l    = self.hiddens[input_idx][layer-1,:,:]
                else:
                    h_prev_l    = self.x[:,input_idx-1,:]

                dx, dh_prev_t   = self.rnn[layer].backward(dh[layer,:,:], h_t, h_prev_l, h_prev_t)
                dh[layer,:,:]   = dh_prev_t

                # Accumulating gradients in previous layers
                if layer > 0:
                    dh[layer-1,:,:] += dx
